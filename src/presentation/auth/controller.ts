            ...
            if (code) {
              ...
              if (serverFound) {
                ...
                if (participanteSelected && participanteSelected.id) {
                  ...
                  if (participanteExistente.length > 0) { //REVISAR ESTE CODIGO
                    const url = parse(req.url || '', true);
                    url.pathname = '/error';
                    url.query.from = '';//url a donde quiero redirigir
                    const errorUrl = format(url);
                ...
                const url = parse(req.url || '', true);
                url.pathname = '/user-notfound';
                url.query.from = '/la-url-a-la-que-quieres-redirigir'; // Reemplaza esto con la URL a la que quieres redirigir
                const notfoundUrl = format(url);
                res.writeHead(302, { Location: notfoundUrl });
                res.end();
